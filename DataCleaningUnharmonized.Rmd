...
## Limit to same participants in both data sets
```{r}
# filter annualized change for IDs in random slope
smri_difference_baseline_y2_scaled %>% 
  filter(src_subject_id %in% ranef_df$src_subject_id)


smri_difference_y2_y4_scaled %>% 
  filter(src_subject_id %in% ranef_df$src_subject_id)
...
ranef_df %>% group_by(rel_family_id) %>% mutate(count = n())
# probably graph distribution

set.seed()

ranef_split <- group_initial_split(
  ranef_df,
  group = rel_family_id,
  prop = 0.80,
...
ranef_train <- training(ranef_split)
ranef_validation <- testing(ranef_split)
```
